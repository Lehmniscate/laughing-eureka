...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    JOIN
      stops c ON a.stop_id = c.id
    JOIN
      stops d ON b.stop_id = d.id
    WHERE
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT
    first_num, first_company, middle_a, second_num, second_company
  FROM (
    SELECT
      DISTINCT
      stopsb.name AS middle_a,
      b.company AS first_company,
      b.num AS first_num
    FROM
      routes a
    JOIN
  ...
  JOIN (
    SELECT
      DISTINCT
      stopsb.name AS middle_b,
      b.company AS second_company,
      b.num AS second_num
    FROM
      routes a
    JOIN
    ...
    JOIN
      stops stopsb ON b.stop_id = stopsb.id
    WHERE
      stopsa.name = 'Sighthill'
  ) leg_two ON middle_a = middle_b
  ORDER BY
    first_num
  SQL
end
