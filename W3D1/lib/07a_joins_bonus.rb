...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, SUM(al.price) / SUM(tracks_per_album)
    FROM
      (
        SELECT
          albums.*,
          COUNT(DISTINCT tracks.song) AS tracks_per_album
        FROM
          albums
        JOIN
          tracks ON albums.asin = tracks.album
    ...
    JOIN
      styles ON styles.album = al.asin
    GROUP BY
      styles.style
    ORDER BY
      SUM(al.price) / SUM(tracks_per_album) DESC
    LIMIT 5
  SQL
end
